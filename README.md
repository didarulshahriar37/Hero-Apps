# WEBSITE NAME 
HERO.IO - We Build Productive Apps
# LIVE SITE LINK
[HERO.IO](https://hero-io-d7.netlify.app/)
# FEATURES
* Trending apps are shown on the home page
* Users can install and uninstall apps.
* Users can browse all apps and installed apps with searching and sorting options.
* Each app shows detailed information including total downloads, ratings and reviews.
* Responsive design for seamless experience across all devices

---

## Technologies used
<div align="left">
  <img width="12" />
  <img src="https://skillicons.dev/icons?i=react" height="40" alt="react logo"  />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" height="40" alt="css3 logo"  />
  <img width="12" />
  <img src="https://skillicons.dev/icons?i=tailwind" height="40" alt="tailwindcss logo"  />
  <img width="12" />
  <img src="https://skillicons.dev/icons?i=vite" height="40" alt="vite logo"  />
  <img width="12" />
  <img src="https://skillicons.dev/icons?i=netlify" height="40" alt="netlify logo"  />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" height="40" alt="git logo"  />
  <img width="12" />
  <img src="https://skillicons.dev/icons?i=github" height="40" alt="github logo"  />
</div>

---

## 🧩 Dependencies

### Core Libraries
[![React](https://img.shields.io/badge/React-19.1.1-blue?style=flat-square)](https://reactjs.org/)  
[![React DOM](https://img.shields.io/badge/React_DOM-19.1.1-blue?style=flat-square)](https://reactjs.org/docs/react-dom.html)  
[![React Router](https://img.shields.io/badge/React_Router-7.9.4-blue?style=flat-square)](https://reactrouter.com/)  

### UI / Styling
[![Tailwind CSS](https://img.shields.io/badge/TailwindCSS-4.1.14-blue?style=flat-square)](https://tailwindcss.com/)  
[![@tailwindcss/vite](https://img.shields.io/badge/@tailwindcss_vite-4.1.14-blue?style=flat-square)](https://www.npmjs.com/package/@tailwindcss/vite)  
[![React Icons](https://img.shields.io/badge/React_Icons-5.5.0-blue?style=flat-square)](https://react-icons.github.io/react-icons/)  

### Components & Utilities
[![React Spinners](https://img.shields.io/badge/React_Spinners-0.17.0-red?style=flat-square)](https://www.npmjs.com/package/react-spinners)  
[![React Toastify](https://img.shields.io/badge/React_Toastify-11.0.5-yellow?style=flat-square)](https://fkhadra.github.io/react-toastify/introduction)  
[![Recharts](https://img.shields.io/badge/Recharts-3.3.0-purple?style=flat-square)](https://recharts.org/)  

---

## Installation
```bash
> git clone https://github.com/didarulshahriar37/Hero-Apps.git
> cd Hero-Apps
> npm install
> npm run dev